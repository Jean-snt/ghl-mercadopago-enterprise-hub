...
Incluye automatización de tags en GoHighLevel para pagos aprobados
"""
import os
import json
import requests
...
"""
    ...
    def notify_payment_approved(self, payment: Payment) -> Dict[str, Any]:
        """Notifica un pago aprobado y aplica tag en GHL automáticamente"""
        try:
            # Aplicar tag en GHL automáticamente
            ghl_tag_result = self._apply_ghl_tag_for_payment(payment)
            
            # Preparar mensaje de notificación
            ghl_status = "✅ Tag aplicado en GHL" if ghl_tag_result.get("success") else "❌ Error aplicando tag en GHL"
            ...
            notification = NotificationMessage(
                ...
                data={
                    ...
                    "ghl_tag_applied": ghl_tag_result.get("success", False),
                    "ghl_tag_name": ghl_tag_result.get("tag_name"),
            ...
            logger.error(f"Error in notify_payment_approved: {str(e)}")
            return {"success": False, "error": str(e)}
    
    def _apply_ghl_tag_for_payment(self, payment: Payment) -> Dict[str, Any]:
        """
        Aplica tag automáticamente en GoHighLevel cuando un pago es aprobado
        """
        try:
            ...
            if not payment.ghl_contact_id:
                logger.warning(f"Payment {payment.id} has no ghl_contact_id")
                return {"success": False, "error": "No GHL contact ID"}
            
            if not payment.client_account_id:
                logger.warning(f"Payment {payment.id} has no client_account_id")
                return {"success": False, "error": "No client account ID"}
            ...
            if not client_account:
                logger.error(f"Client account {payment.client_account_id} not found")
                return {"success": False, "error": "Client account not found"}
            
            # Determinar el tag a aplicar
            default_tag = "Pago confirmado"
            tag_name = getattr(client_account, 'default_tag_paid', default_tag) or default_tag
            
            # Inicializar servicio GHL
            ghl_service = GHLOAuthService(self.db)
            
            # Obtener token válido
            access_token = ghl_service.get_valid_token(client_account.client_id)
            if not access_token:
                logger.error(f"No valid GHL token for client {client_account.client_id}")
                return {"success": False, "error": "No valid GHL token"}
            
            # Aplicar tag usando la API de GHL
            tag_result = self._call_ghl_tag_api(
                access_token=access_token,
                contact_id=payment.ghl_contact_id,
                tag_name=tag_name,
                payment=payment
            )
            ...
            if tag_result.get("success"):
                ...
                self._log_ghl_tag_event(payment, tag_name, "success")
                logger.info(f"GHL tag '{tag_name}' applied successfully to contact {payment.ghl_contact_id} for payment {payment.id}")
                
                return {
                    "success": True,
                    "tag_name": tag_name,
                    "contact_id": payment.ghl_contact_id,
                    "message": f"Tag '{tag_name}' aplicado exitosamente"
                }
            else:
                # Registrar error en audit log
                self._log_ghl_tag_event(payment, tag_name, "failed", tag_result.get("error"))
                logger.error(f"Failed to apply GHL tag: {tag_result.get('error')}")
            ...
            logger.error(f"Error applying GHL tag for payment {payment.id}: {str(e)}")
            return {"success": False, "error": str(e)}
    
    def _call_ghl_tag_api(
        self,
        access_token: str,
        contact_id: str,
        tag_name: str,
        ...
        """
        Llama a la API de GoHighLevel para aplicar tag al contacto
        """
        try:
            # Detectar si es modo simulación
            is_simulation = access_token.startswith("mock_ghl_access_token_")
            
            if is_simulation:
                # Modo simulación - simular éxito
                logger.info(f"🎭 SIMULACIÓN: Aplicando tag '{tag_name}' al contacto {contact_id}")
                return {
                    "success": True,
                    "message": f"Tag '{tag_name}' aplicado en modo simulación",
                    "simulation": True
                }
            
            # Modo producción - llamada real a GHL API
            headers = {
            ...
            # Preparar datos para actualizar el contacto
            update_data = {
                "tags": [tag_name]
            }
            
            # URL de la API de contactos de GHL
            ...
            if response.status_code in [200, 201]:
                ...
                return {
                    "success": False,
                    "error": f"GHL API error: {response.status_code} - {response.text}",
                    "response_code": response.status_code
                }
    ...
    def _log_ghl_tag_event(
        ...
        """
        Registra evento de aplicación de tag en GHL en los logs de auditoría
        """
        try:
            ...
            event_details = {
                ...
                "amount": str(payment.paid_amount) if payment.paid_amount else None
            }
            
            if error_message:
                event_details["error"] = error_message
            ...
            audit_log = AuditLog(
                ...
                entity_id=payment.ghl_contact_id,
                changes=event_details,
                created_by="system_auto_tag",
                correlation_id=f"ghl_tag_{payment.id}_{int(time.time())}"
            )
            
            self.db.add(audit_log)
            self.db.commit()
            
            # Log específico para el evento
            if status == "success":
                logger.info(f"PaymentEvent: Tag '{tag_name}' aplicado exitosamente en GHL para el contacto {payment.ghl_contact_id}")
            else:
                logger.error(f"PaymentEvent: Error aplicando tag '{tag_name}' en GHL para el contacto {payment.ghl_contact_id}: {error_message}")
                
        except Exception as e:
            logger.error(f"Error logging GHL tag event: {str(e)}")
